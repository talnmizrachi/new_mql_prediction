...
def extract_columns_data(_df, _starting_level):
    ...
    cat_cols, numerical = get_sorted_funnel_for_(_starting_level, text_features)
...
if __name__ == '__main__':
    ...
    features, text_features, all_features, non_features, cat_cols, numeric_col, final_columns = extract_columns_data(df, starting_level)
    ...
    for code, columns in minus_one_dict().items():
        ...
        final_columns = list(columns) + [target]
        ...
        if precision >= max_precision and recall > max_recall and (precision != max_precision or recall != max_recall):
            ...
            max_recall = recall
            task.connect({"precision": precision, "recall": precision, "code":code})
            print(columns)
            if code == "c7ad10efb9314a3095962deb12b7171d":
                task.add_tags(["all_cols"])
            task.add_tags(["Winner"])
            task.close()
            task.create(project_name="my project", task_name=f"after_{winner_code}")
            winner_code = code
